...
class App(pyglet.window.Window):
    def __init__(self):
        ...
        self.info_label = pyglet.text.Label("", multiline=True, x=50, y=self.height - 50,
            width=self.width-100, height=self.height-100, anchor_y="top", font_name="monospace", font_size=19)
    ...
    def node_name(self,label, x, y):
        node_label = pyglet.text.Label(label, font_name='Sans', font_size=28, x=x+10, y=y+10)
        node_label.color = (0, 0, 0, 255)
        return node_label
    ...
    def check_edge(self, x, y):
        ...
        for edge in self.g.edges():
            ...
            r = ((n1x - n2x)**2 + (n1y - n2y)**2) / 3.0 # squared radius
        ...
        if self.help:
            ...
            for edge in self.g.edges():
                ...
                pyglet.graphics.draw(2, pyglet.gl.GL_LINES, ('v2f', (
                    (ox + self.g.nodes[edge[0]]["x"] * self.scale)+10,
                    (oy + self.g.nodes[edge[0]]["y"] * self.scale)+15,
                    (ox + self.g.nodes[edge[1]]["x"] * self.scale)+10,
                    (oy + self.g.nodes[edge[1]]["y"] * self.scale)+15)))

            # draw nodes
            node_count = 0
            for node in self.g.nodes():
                 
                # self.node_label.draw()
                ...
                self.node_name(str(node_count),ox + self.g.nodes[node]["x"] * self.scale,oy + self.g.nodes[node]["y"] * self.scale).draw()
    ...
    def on_key_press(self, symbol, modifiers):
        if symbol == key.H:
            ...
            node_sum = []
            node_sum_r = 0
            for node_r in self.g.nodes():
                for node_c in self.g.nodes():
                    edges = (node_r,node_c)
                    in_edges = (node_c,node_r)
                    if edges in self.g.edges() or in_edges in self.g.edges():
                        out = out + str(" 1 ")
                        node_sum_r = node_sum_r + 1
                    else:
                        out = out + str(" 0 ")
                out = out + str('\n')
                node_sum.append(node_sum_r)
                node_sum_r = 0
            
            print(out)
            degree = ""
            node_co = 0
            for n_sum in node_sum:
                degree = degree + ("degree of node "+str(node_co) +" = "+ str(n_sum) + '\n')
                node_co = node_co + 1


            self.info_label.text = "Info\nNumber of nodes: {0}\nNumber of edges: {1}\nadjacency matrix\n\n{2}\n{3}".format(node_number, edge_number,out,degree)
